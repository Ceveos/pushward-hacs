...
def _schema_keys(schema: vol.Schema) -> set[str]:
    """Return plain field names from a voluptuous schema."""
    return {str(key.schema if isinstance(key, vol.Marker) else key) for key in schema.schema}


def test_entity_staged_schemas_are_complete_and_disjoint() -> None:
    """The guided pages must expose every field from the original details form exactly once."""
    full = _details_schema("sensor.temperature", "timeline", defaults={})
    lifecycle, content, appearance = _entity_staged_schemas("sensor.temperature", "timeline", defaults={})
    stage_keys = [_schema_keys(schema) for schema in (lifecycle, content, appearance)]
    assert stage_keys[0].isdisjoint(stage_keys[1])
    assert stage_keys[0].isdisjoint(stage_keys[2])
    assert stage_keys[1].isdisjoint(stage_keys[2])
    assert set().union(*stage_keys) == _schema_keys(full)


def test_compound_layouts_use_repeatable_form_selectors() -> None:
    """Timeline, board, and stat-list inputs render as forms rather than mini-languages."""
    timeline = _details_schema("sensor.temperature", "timeline", defaults={})
    board = _details_schema("binary_sensor.home", "board", defaults={})
    _, timeline_series_validator = next(
        (key, validator)
        for key, validator in timeline.schema.items()
        if str(key.schema if isinstance(key, vol.Marker) else key) == CONF_SERIES_ENTITIES
    )
    _, board_tiles_validator = next(
        (key, validator)
        for key, validator in board.schema.items()
        if str(key.schema if isinstance(key, vol.Marker) else key) == CONF_TILES
    )
    widget_content, _, _ = _widget_staged_schemas(
        "sensor.users", WIDGET_TEMPLATE_STAT_LIST, defaults={}
    )
    _, stat_rows_validator = next(
        (key, validator)
        for key, validator in widget_content.schema.items()
        if str(key.schema if isinstance(key, vol.Marker) else key) == CONF_STAT_ROWS
    )
    assert isinstance(timeline_series_validator, ObjectSelector)
    assert timeline_series_validator.config["multiple"] is True
    assert isinstance(board_tiles_validator, ObjectSelector)
